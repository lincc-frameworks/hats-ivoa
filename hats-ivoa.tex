...
\caption{Adaptive tiling for the object catalog of Zwicky Transient Facility, Data Release 14 \citep{ztf:Bellm2019, ztf:Masci2019} To achieve a similar number of rows across the whole sky, we further partition the galactic region, resulting in tiles of higher HEALPix order in the galactic plane, especially near the galactic bulge. }
\label{fig:order}
\end{figure}
...
In this way, you can query the index catalog for the HEALPix pixel where the value can be found, and then only need to load the primary catalog partition for the corresponding HEALPix pixel. 
This greatly speeds up and facilitates the query, as we now only have to load a single pixel instead of going through the whole catalog.
