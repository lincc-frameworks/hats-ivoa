...
HATS is inspired by the HiPS standard \footnote{\href{https://www.ivoa.net/documents/HiPS/20170519/index.html}{https://www.ivoa.net/documents/HiPS/20170519/index.html}}.
HATS shares similar principles and structure, but serves a different purpose.
This section briefly tackles both approach's similarities and discrepancies.
...
Property files are made of a simple list of key/value pairs, easy to parse and to interpret.
Such files can for example be seen as Java properties, or simple TOML, files.
HiPS and HATS both rely on well supported -- and even standardized -- HEALPix files: MOC for HiPS, Skymaps for HATS.

But we can note two main differences.
\begin{enumerate}
...
It means that for a tile at a given depth, a part of its information is available in parent cells.
Moreover, each tile (including leaf tiles) is quite small (a few tens to a few hundreds of kilobytes).
On the contrary, HATS is an adaptative grid with information distributed in the HEALPix NESTED tree leaves, and no degraded nor redundant information on nodes.
Each leaf is quite large (typically a few hundreds of megabytes). 
The main goal is the possibility to perform operations (including row-filtering) on sub-columns, on the whole dataset (often meaning the whole sky).
   \item Secondly, on one hand, HiPS does not rely on any given external implementation.
The implementer of a client supporting the standard has to write its own, moderately complex, code.
The moderate size of a HEALPix tile is guaranteed by construction in the standard.
On the other hand, HATS relies on an industry standard and thus on the external implementation of a complex code (the Parquet file format and distant interrogation code). 
...
\begin{itemize}
 ...
 \begin{itemize}
  ...
  \item HATS: \texttt{partition\_info.csv} (recommended) and \texttt{point\_maps.fits} (density map, optional)
 \end{itemize}
...
However, the HiPS standard is directed towards progressive visualization -- meaning that each HEALPix order contains information -- while the
HATS format optimizes operations on (and between) catalogs -- and thus stores information at the most precise order without redundancy.
