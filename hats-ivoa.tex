...
\begin{itemize}
    ...
    HATS provides such an indexing scheme, and creates files that have both properties and size that is well-suited to parallel operations.
...
The user can query the index catalog for the HEALPix tile where the value can be found, and then need only load the primary catalog partition for the corresponding HEALPix tile. 
This greatly speeds up and facilitates the query, as we now have to load only single tile instead of going through the whole catalog.
